...
def print_results(models, features, X_test, y_test, path):
    ...
    for i in range(len(models)):
        ...
        results.append(
                [models[i].__class__.__name__, str(round(metrics.accuracy_score(),3)), str(round(metrics.f1_score(),3)), str(round(metrics.f2_score(),3)),
                 str(round(metrics.recall_score(),3)), str(round(metrics.precision_score(),3)),
                 str(round(metrics.informedness_score(),3)), str(round(metrics.rmse(),3)),
